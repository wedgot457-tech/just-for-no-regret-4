...
// Media is stored separately from localStorage so selected photos/videos survive
// closing and reopening the editor without putting large binary data into localStorage.
const MEDIA_FIELDS = ['bgFile','letterPhoto','yesPhoto','noPhoto','momentPhoto'];
let mediaDBPromise = null;
function mediaDB(){
  if(mediaDBPromise) return mediaDBPromise;
  mediaDBPromise = new Promise((resolve,reject)=>{
    const req=indexedDB.open('justForNoRegretMedia',1);
    req.onupgradeneeded=()=>{ if(!req.result.objectStoreNames.contains('files')) req.result.createObjectStore('files'); };
    req.onsuccess=()=>resolve(req.result); req.onerror=()=>reject(req.error);
  });
  return mediaDBPromise;
}
async function storeMedia(field,file){try{const db=await mediaDB();await new Promise((res,rej)=>{const tx=db.transaction('files','readwrite');tx.objectStore('files').put(file,field);tx.oncomplete=res;tx.onerror=()=>rej(tx.error);});}catch(e){console.warn('Media local save failed:',e);}}
async function getMedia(field){try{const db=await mediaDB();return await new Promise((res,rej)=>{const tx=db.transaction('files','readonly');const r=tx.objectStore('files').get(field);r.onsuccess=()=>res(r.result||null);r.onerror=()=>rej(r.error);});}catch{return null;}}
async function clearMedia(){try{const db=await mediaDB();await new Promise((res,rej)=>{const tx=db.transaction('files','readwrite');tx.objectStore('files').clear();tx.oncomplete=res;tx.onerror=()=>rej(tx.error);});}catch{}}
async function restoreStoredMedia(){
  if(window.SHARED) return;
  for(const field of MEDIA_FIELDS){
    const file=await getMedia(field); if(!file) continue;
    const url=URL.createObjectURL(file);
    if(field==='bgFile'){
      state.bgUrl=url; state.bgType=file.type.startsWith('video/')?'video':'image';
      updateMediaPreview('bgFile',url,state.bgType); applyBg(url,state.bgType);
    }else{
      state[`${field}Url`]=url; updateMediaPreview(field,url,'image');
      if(field==='letterPhoto'&&$('letterPhotoImg')){$('letterPhotoImg').src=url;$('letterPhotoImg').classList.add('show');}
    }
  }
  saveLocal();
}
...
function apply() {
  ...
  if (!window.SHARED) {
    const previewMap={bgFile:'bgPreview',letterPhoto:'letterPhotoPreview',yesPhoto:'yesPhotoPreview',noPhoto:'noPhotoPreview',momentPhoto:'momentPhotoPreview'};
    for(const [field,pid] of Object.entries(previewMap)){
      const box=$(pid); if(!box) continue;
      const url=field==='bgFile'?state.bgUrl:state[`${field}Url`];
      box.innerHTML='';
      if(url){
        if(field==='bgFile' && state.bgType==='video'){
          const v=document.createElement('video');v.src=url;v.muted=true;v.playsInline=true;v.controls=false;box.appendChild(v);
        }else{
          const img=document.createElement('img');img.src=url;img.alt='Selected media';box.appendChild(img);
        }
        const label=document.createElement('span');label.textContent=/^https:\/\//i.test(String(url))?'Saved':'Preview';box.appendChild(label);
      }else box.innerHTML='<span>No media selected</span>';
    }
...
function updateMediaPreview(fieldId,url,type){
  const box=$(fieldId==='bgFile'?'bgPreview':`${fieldId}Preview`); if(!box)return;
  box.innerHTML='';
  if(!url){box.innerHTML='<span>No media selected</span>';return;}
  const el=document.createElement(fieldId==='bgFile'&&type==='video'?'video':'img');
  el.src=url;el.alt='Selected media';
  if(el.tagName==='VIDEO'){el.muted=true;el.playsInline=true;el.controls=false;el.autoplay=true;}
  box.appendChild(el);
  const label=document.createElement('span');label.textContent=/^https:\/\//i.test(String(url))?'Saved':'Preview';box.appendChild(label);
}

async function persistSelectedFile(fieldId, kind, file, applyPreview) {
  if (!file) return;

  // Always show the selected file immediately. The network upload is a second
  // step, so a slow/failing Blob request can never make the editor look empty.
  await storeMedia(fieldId,file);
  const localUrl = file.type.startsWith('video/') ? URL.createObjectURL(file) : await new Promise((resolve,reject)=>{
  ...
  if (fieldId === 'bgFile') {
    ...
    state.bgUrl = localUrl;
    state.bgType = type;
    updateMediaPreview('bgFile', localUrl, type);
    state.yt = '';
    if ($('yt')) $('yt').value = '';
    applyYoutube('');
    applyBg(localUrl, type);
  } else {
    state[`${fieldId}Url`] = localUrl;
    updateMediaPreview(fieldId, localUrl, 'image');
    if (fieldId === 'letterPhoto' && $('letterPhotoImg')) {
      $('letterPhotoImg').src = localUrl;
      $('letterPhotoImg').classList.add('show');
    }
    if (applyPreview) applyPreview(localUrl);
  }
  saveLocal();

  try {
    toast(`Uploading ${kind === 'background-video' ? 'background video' : 'image'}…`);
    const url = await uploadFileToBlob(file, kind);
    if (!url) throw new Error('No Blob URL returned.');

    if (fieldId === 'bgFile') {
      state.bgUrl = url;
      state.bgType = file.type.startsWith('video/') ? 'video' : 'image';
      updateMediaPreview('bgFile', url, state.bgType);
      applyBg(url, state.bgType);
    } else {
      state[`${fieldId}Url`] = url;
      updateMediaPreview(fieldId, url, 'image');
      if (fieldId === 'letterPhoto' && $('letterPhotoImg')) {
        $('letterPhotoImg').src = url;
        $('letterPhotoImg').classList.add('show');
      }
      if (applyPreview) applyPreview(url);
    }
    saveLocal();
    toast('Saved automatically.');
  } catch (error) {
    console.error('Media upload failed:', error);
    // Keep the local object-URL preview alive for this session. Sharing will
    // report the upload error rather than creating a broken public link.
    toast(error?.message || 'Upload failed. Your local preview is still available.');
  }
}
function bindFiles(){
  if(window.SHARED)return;
  ...
  if(bg)bg.onchange=e=>{
    ...
    const kind=f.type.startsWith('video/')?'background-video':'background-image';
    persistSelectedFile('bgFile', kind, f);
  };

  ['letterPhoto','yesPhoto','noPhoto','momentPhoto'].forEach(id=>{
    $(id)?.addEventListener('change',e=>{
      const f=e.target.files?.[0]; if(!f)return;
      persistSelectedFile(id, 'photo', f);
    });
  });
}
...
async function uploadFileToBlob(file,kind){
  ...
  if(!contentType.startsWith('image/') && !contentType.startsWith('video/')) throw new Error('Unsupported media type.');

  // Use Vercel's official client-upload flow. The browser uploads directly to
  // Blob after the API issues a scoped client token. This avoids the 4.5 MB
  // Vercel Function body limit and also avoids hand-built signed PUT requests.
  if(!window.__vercelBlobUpload){
    const mod=await import('https://cdn.jsdelivr.net/npm/@vercel/blob@2.6.1/client/+esm');
    if(typeof mod.upload!=='function') throw new Error('Vercel Blob client could not load.');
    window.__vercelBlobUpload=mod.upload;
  }
  const upload=window.__vercelBlobUpload;
  const pathname=`letters/media/${Date.now()}-${String(file.name||'upload').replace(/[^a-zA-Z0-9._-]/g,'-')}`;
  const result=await upload(pathname,file,{
    access:'public',
    contentType,
    handleUploadUrl:'/api/upload',
    clientPayload:JSON.stringify({contentType,size:file.size,kind:kind||'media'}),
    multipart:file.size>4*1024*1024,
    onUploadProgress:(event)=>{
      if(event?.percentage!=null) toast(`Uploading… ${Math.round(event.percentage)}%`);
    }
  });
  if(!result?.url || !/^https:\/\//i.test(result.url)) throw new Error('Blob upload finished without a public URL.');
  return result.url;
}
async function uploadForShare(fieldId,kind,savedUrl){
  const file=$(fieldId)?.files?.[0] || await getMedia(fieldId);
...
async function createShare(){
  ...
  try{
    ...
    const copy={...state};
    delete copy.bgUrl; delete copy.bgType;
    for(const k of ['letterPhoto','yesPhoto','noPhoto','momentPhoto']) delete copy[`${k}Url`];

    const tasks=[
      ['letterPhoto','photo',state.letterPhotoUrl,'Uploading letter photo…'],
      ['yesPhoto','photo',state.yesPhotoUrl,'Uploading YES photo…'],
      ['noPhoto','photo',state.noPhotoUrl,'Uploading NO photo…'],
      ['momentPhoto','photo',state.momentPhotoUrl,'Uploading moment photo…'],
      ['bgFile',$('bgFile')?.files?.[0]?.type?.startsWith('video/')?'background-video':'background-image',state.bgUrl,'Uploading background…']
    ];

    const results=[];
    for(const [field,kind,saved,message] of tasks){
      const input=$(field);
      const file=input?.files?.[0] || await getMedia(field);
      if(typeof saved==='string' && /^https:\/\//i.test(saved)) {
        results.push(saved);
      } else if(file) {
        toast(message);
        results.push(await uploadFileToBlob(file,kind));
      } else {
        results.push(null);
      }
    }
    ...
    if(bgUrl){
      copy.bgUrl=bgUrl;
      copy.bgType=state.bgType || ($('bgFile')?.files?.[0]?.type?.startsWith('video/')?'video':'image');
    }

    // Persist any newly returned URLs before creating the share record.
    if(letterPhotoUrl)state.letterPhotoUrl=letterPhotoUrl;
    if(yesPhotoUrl)state.yesPhotoUrl=yesPhotoUrl;
    if(noPhotoUrl)state.noPhotoUrl=noPhotoUrl;
    if(momentPhotoUrl)state.momentPhotoUrl=momentPhotoUrl;
    if(bgUrl){state.bgUrl=bgUrl;state.bgType=copy.bgType;}
    saveLocal();
...
async function loadShared(){
  ...
  try{
    ...
    let r=await fetch(`/api/share?slug=${encodeURIComponent(slug)}`,{cache:'no-store'});let j={};try{j=await r.json();}catch{}
    // Keep the dynamic endpoint as a fallback for older deployments.
    if(!r.ok){r=await fetch(`/api/share/${encodeURIComponent(slug)}`,{cache:'no-store'});j={};try{j=await r.json();}catch{}}
...
async function initEditor(){
  ...
  $('reset')?.addEventListener('click',async()=>{state={...defaults};await clearMedia();saveLocal();apply();toast('Reset complete.');});
  ...
  bindFiles();apply();await restoreStoredMedia();
}
