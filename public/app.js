...
async function persistSelectedFile(fieldId, kind, file, applyPreview) {
  ...
  try {
    ...
    // The important part: save the permanent Blob URL, not the temporary
    // object/data URL. This survives closing and reopening the editor.
    saveLocal();
    toast('Saved automatically.');
  } catch (error) {
    console.error('Media upload failed:', error);
    // Keep the local preview working even if the network upload fails.
    if (fieldId === 'bgFile') {
      const type = file.type.startsWith('video/') ? 'video' : 'image';
      if (type === 'video') {
        const u = URL.createObjectURL(file);
        state.bgUrl = u; state.bgType = 'video'; state.yt = '';
        applyYoutube(''); applyBg(u, 'video');
      } else {
        filePreview(file, (u) => { state.bgUrl = u; state.bgType = 'image'; state.yt = ''; applyYoutube(''); applyBg(u, 'image'); });
      }
    } else {
      filePreview(file, (u) => {
        state[`${fieldId}Url`] = u;
        if (fieldId === 'letterPhoto' && $('letterPhotoImg')) {
          $('letterPhotoImg').src = u; $('letterPhotoImg').classList.add('show');
        }
      });
    }
    saveLocal();
...
async function uploadFileToBlob(file,kind){
  ...
  // The browser sends the file directly to Blob. The Vercel Function only
  // creates the short-lived signed PUT URL.
  const put=await fetch(data.uploadUrl,{method:'PUT',headers:{'content-type':meta.contentType},body:file});
  if(!put.ok){
    let detail='';try{detail=await put.text();}catch{}
    throw new Error(detail||`Direct Blob upload failed (${put.status})`);
  }

  // API returns the immutable public Blob URL, never the temporary signed URL.
