...
async function uploadFileToBlob(file,kind){
  ...
  // Small files go through our Vercel Function -> Blob path. Larger files use
  // the signed direct-to-Blob path so they never hit the 4.5 MB Function limit.
  if(file.size <= SERVER_LIMIT){
    const r=await fetch('/api/upload',{
      method:'PUT',
      headers:{
        'content-type':contentType,
        'x-upload-kind':kind||'media',
  ...
  const meta={filename:file.name||'upload',contentType,size:file.size,kind:kind||'media'};
  const r=await fetch('/api/upload',{method:'POST',headers:{'content-type':'application/json','cache-control':'no-store'},body:JSON.stringify(meta)});
  let data={};try{data=await r.json();}catch{}
  if(!r.ok||!data.uploadUrl) throw new Error(data.error||`Upload preparation failed (${r.status})`);
  const put=await fetch(data.uploadUrl,{method:'PUT',body:file});
  if(!put.ok){let detail='';try{detail=await put.text();}catch{}throw new Error(detail||`Direct Blob upload failed (${put.status})`);}
  if(!data.url||!/^https:\/\//i.test(data.url)) throw new Error('Blob upload finished but no public URL was returned.');
  return data.url;
}
...
async function createShare(){
  ...
  try{
    ...
    for(const [field,kind,saved,message] of tasks){
      ...
      // If media was already uploaded when selected, its https URL is already
      // permanent and can be reused. If a file is still selected, upload it again
      // only when the saved URL is missing.
      if(file && !(typeof saved==='string' && /^https:\/\//i.test(saved))){
        toast(message);
        results.push(await uploadFileToBlob(file,kind));
      } else if(typeof saved==='string' && /^https:\/\//i.test(saved)) {
        results.push(saved);
      } else if(typeof saved==='string' && /^data:/i.test(saved)) {
        toast(message);
        const blob=await (await fetch(saved)).blob();
        const ext=(blob.type||'image/jpeg').split('/')[1]||'bin';
        results.push(await uploadFileToBlob(new File([blob],`restored.${ext}`,{type:blob.type||'application/octet-stream'}),kind));
      } else {
        results.push(null);
      }
    }
