...
async function uploadFileToBlob(file,kind){
  ...
  // Only this small JSON request touches the Vercel Function. The actual file
  // is PUT directly from the browser to Vercel Blob, avoiding the 4.5 MB
...
async function createShare(){
  ...
  try{
    ...
    const tasks=[
      ...
      ['bgFile',bgKind,state.bgUrl,bgKind==='background-video'?'Uploading background video…':'Uploading background…']
    ];
    const results=[];
    for(const [field,kind,saved,message] of tasks){
      const input=$(field);
      const hasFile=!!input?.files?.[0];
      const hasSaved=typeof saved==='string' && (saved.startsWith('data:') || saved.startsWith('https://'));
      if(hasFile || hasSaved){
        toast(message);
        results.push(await uploadForShare(field,kind,saved));
